...
# Find index
 yy <- 2015; mm <- 01; dd <- 05
 ind <- which(year(Time)==yy & month(Time)==mm & day(Time)==dd)                 # find index
...
#------- Change Limits
# x_left <- which (lon >= 130 & lon <= 130.1); y_top <- which (lat >= 42 & lat <= 42.1)
# map1 <- basemap("EastSea", limits=c(lon[x_left],lon[length(lon)],lat[y_top],lat[length(lat)]), bathymetry=TRUE) +  # Chg Limit
  
  # geom_segment(data = data1, aes(x = lon1, y = lat1, xend = lon1 + u2, yend = lat1 + v2, group = Velocity, color = Velocity),
  #              arrow = arrow(length = unit(0.1,"cm"))) + 
  geom_segment(data = data1, aes(x = lon1, y = lat1, xend = lon1 + u2, yend = lat1 + v2)) +
...
ggsave("D:/RScript/Figure/Wind_vector_black.png", device="png")
# ggsave("D:/RScript/Figure/Wind_vector_black_limit.png", device="png")
